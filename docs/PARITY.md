...
| Sessions | Every admitted top-level DM or group message creates a thread root and isolated Hermes session; active thread follow-ups reuse it without a mention; an existing thread imports every preceding message and resource on first mention and fails visibly if the snapshot is incomplete | Migration of existing OpenClaw session history |
...
| CardKit | Thinking, Generating, tool running/success/failure, banner-free successful completion, Error, cumulative content streaming, Hermes approval actions, and ordered display segments across accepted steer, blocking-interaction, and native-artifact boundaries | Physical client clicks, footer parity, and reasoning-delta presentation |
